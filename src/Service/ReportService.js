...
class ReportService {
    async getAll() {
        try {
            ...
            const getChatGPT = await ReportRepository.getChatGPT(date)
            let month = []
            for (let i = 0; i < 4; i++) {
            ...
            let chatGPTData = JSON.parse(JSON.stringify(inchatData))
                ...
                for (var j in getChatGPT) {
                    if (chatGPTData[i].bot_id == getChatGPT[j].bot_id)
                        for (let w = 0; w <= 3; w++) {
                            ...
                            chatGPTData[i].months[w].chatgpt = getChatGPT[j].chatgpt !== undefined ? getChatGPT[j].chatgpt : 0
                        }
                }
            }
